...
void but1_callback(void) {
  ...
  int delay = +100;
  xQueueSendFromISR(xQueueButValue, (void *)&delay,  &xHigherPriorityTaskWoken);
}
...
static void task_but(void *pvParameters) {
  ...
  for (;;) {
	if (xQueueReceive(xQueueButValue, &temp, (TickType_t)0)) {
		delayTicks += temp;
...
int main(void) {
  ...
  xQueueButValue = xQueueCreate(32, sizeof(uint32_t));
  if (xQueueButValue == NULL)
    printf("falha em criar a queue \n");
